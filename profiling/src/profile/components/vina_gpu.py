...
def apply_rules(combo: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Keep thread count fixed; quality still flows into metadata for bookkeeping.
    c["thread"] = 8000
    ...
    try:
        ...
        print(f"[WARN] vina_gpu input resolution failed for {c.get('scaffold')} {c.get('ligand')}: {exc}")
        return None
